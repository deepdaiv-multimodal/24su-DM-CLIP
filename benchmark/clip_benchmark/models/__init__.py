...
def load_clip(model_type: str, model_name: str, pretrained: str, cache_dir: str, image_encoder_id: str, device: Union[str, torch.device] = "cuda"):
    assert model_type in MODEL_TYPES, f"model_type={model_type} is invalid!"
    load_func = TYPE2FUNC[model_type]
    return load_func(model_name=model_name, pretrained=pretrained, cache_dir=cache_dir, image_encoder_id=image_encoder_id, device=device)