...
import numpy as np
from benchmark.clip_benchmark.models import load_clip
import torch
from torch.utils.data import DataLoader

from benchmark.clip_benchmark.datasets.builder import (build_dataset, dataset_collection,
                                             get_dataset_collate_fn,
                                             get_dataset_collection_from_file,
                                             get_dataset_default_task)
from benchmark.clip_benchmark.metrics import (captioning, image_caption_selection,
                                    linear_probe, zeroshot_classification,
                                    zeroshot_retrieval)
from benchmark.clip_benchmark.model_collection import (get_model_collection_from_file,
                                             model_collection)
from benchmark.clip_benchmark.models import MODEL_TYPES, load_clip
...
def get_parser_args():
    ...
    parser_eval.add_argument('--image_encoder_id', default=None, type=str, help="image encoder id for mobile_clip models")
...
def measure_latency(model_part, sample, num_iterations=100):
    latency_list = []
...
def run(args):
    ...
    with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.bfloat16):
        image_encoder_latency = measure_latency(model.encode_image, sample_image)
        text_encoder_latency = measure_latency(model.encode_text, tokenized_sample_text)

    fps_info = {
        "image_encoder_latency": image_encoder_latency,
        "text_encoder_latency": text_encoder_latency
    }
